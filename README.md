...
*   **Dynamic Spatial Zones (IoA Tracking)**: Draw custom polygons on the camera feed. The system uses homography to align zones if the camera is bumped, and utilizes robust **Intersection-over-Area (IoA)** pixel-math to detect occupancy. This ensures precise dwell tracking even if a person's feet are occluded (e.g., sitting at a desk) while completely ignoring accidental arm-brushing.
