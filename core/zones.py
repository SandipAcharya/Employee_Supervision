...
class ZoneChecker:
    ...
    def check_box_overlap(self, box, frame_shape, overlap_threshold=0.20):
        ...
        # --- Core Mass Isolation ---
        # A person's bounding box expands wildly if they stretch their arms out.
        # We shrink the width by 25% on each side to isolate the "torso/core",
        # preventing false triggers from outstretched hands.
        width = x2 - x1
        core_x1 = int(x1 + width * 0.25)
        core_x2 = int(x2 - width * 0.25)
        
        # Clip coordinates to frame boundaries
        h, w = frame_shape[:2]
        core_x1, y1 = max(0, core_x1), max(0, y1)
        core_x2, y2 = min(w, core_x2), min(h, y2)
        
        core_area = (core_x2 - core_x1) * (y2 - y1)
        if core_area <= 0: return None
        ...
        for name, poly in self.aligned_zones.items():
            ...
            # Crop the mask exactly to the person's core bounding box
            overlap_crop = mask[y1:y2, core_x1:core_x2]
            overlap_area = cv2.countNonZero(overlap_crop)
            
            poly_area = cv2.contourArea(poly)
            
            # If the person's core body is 20% inside the zone OR they cover 20% of the desk area
            if overlap_area > (core_area * overlap_threshold) or (poly_area > 0 and overlap_area > (poly_area * overlap_threshold)):
                return name
